...
from .cameca_ks.cameca import Cameca
...
def read_c_hash_string(stream):
    ...
    return stream.read(str_len).decode("cp1252")
...
class CamecaBase(object):
    def _read_the_header(self, fbio):
        ...
        for i in range(n_changes):
            filetime = unpack('<Q', fbio.read(8))[0]
            comment = read_c_hash_string(fbio)
...
class CamPeakSetup:
    def __init__(self, stream, index, sxf_version):
        ...
        self.XTAL = xtal.decode('cp1252')[::-1]
        stream.seek(8, 1)
