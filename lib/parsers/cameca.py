...
Software reads data produced by Cameca(TM) Peaksight(TM) software.

It is developed independantly from Cameca or
Ametek inc.
Development is based on the RE of binary data formats which
is Fair Use.

The code is intended to be used with python 3 and Qt5 libraries.
...
class CamecaBase(object):
    ...
    definition_nodes = {
                        0: 'point', 1: 'line stage', 2: 'line beam',
    ...
    def to_definition_type(cls, node_type):
        """return the string representation of caneca aquisition definition"""
        return cls.definition_nodes[node_type]
...
class DatasetItem(CamecaBase):
    def __init__(self, fbio):
        ...
        values = struct.unpack('<5i2f2i', fbio.read(36))
...
class WDSDatasetItem(DatasetItem):
    def __init__(self, fbio):
        DatasetItem.__init__(self, fbio)
    ...
    def read_item(self, fbio):
        ...
        print('size: ', size, ' ; offset: ', fbio.tell())
